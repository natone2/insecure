...
class initialize(core):
	...
	def __init__(self, mode):
		...
		self._config = {
				...
				'workspaces_directory_name': '.insecure/workspaces'
			}
	...
	def _check_version(self):
		...
		try:
			remote = re.search(pattern, self.request(\
				'https://raw.githubusercontent.com/saeeddhqan/insecure/master/insecure').text).group(1)
			local = re.search(pattern, open('insecure').read()).group(1)
		except Exception as e:
			self.error(f"Version check failed ({type(e).__name__}).")
		if remote != local:
			self.alert('Your version of insecure does not match the latest release')
	...
	def show_banner(self):
		print(f"OWASP insecure({__version__}): Open-source Intelligence Framework.\
