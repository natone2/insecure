...
meta = {
	...
	'examples': ('creativecommons -q <QUERY> -l 15 --output',)
}

def module_api(self):
    query = self.options['query']
    limit = self.options['limit']
    run = self.creativecommons(query, limit)
    run.run_crawl()
    output = {}
    ...
    self.save_gather(output, 'search/creativecommons', query, output=self.options['output'])
    return output
